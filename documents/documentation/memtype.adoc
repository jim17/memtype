...
WARNING: When you execute the MemTypeTool GUI, the first thing to do is to `unlock` the
memtype (if it's still locked) as it needs to be properly unlocked by `PIN` in
order to accept any command from the GUI. 
